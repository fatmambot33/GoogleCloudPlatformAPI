...
def test_list_views():
    with patch.object(
        analytics_mod.Analytics, "ServiceAccount"
    ) as mock_service_account, patch.object(analytics_mod, "build") as mock_build:
...
def test_get_report():
    with patch.object(
        ...
        fake_reporting.reports.return_value.batchGet.return_value.execute.return_value = (
            mock_report
        )
